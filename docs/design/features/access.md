...
ファイル操作はHTTPごとに新しい世代を採用せず、操作開始時の世代を読込み・ハッシュ計算・複数要求・本文解析・端末保存・画面の完了反映まで保持する。詳細は[証跡設計](evidence.md)。共通HTTP処理も送信前・初回応答後・更新待ち完了後に取消signalを確認し、取り消された要求を再送しない。同じ世代の別要求が共有するSDK更新は取り消さない。
